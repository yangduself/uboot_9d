...
int s3c_usb_phy_inited=0;
int s3c_usb_connected_status= 0;/*0,none,1,usb connected,2,power connected,3,bad usb connected*/
...
void s3c_usb_init_device(void)
{
	...
	s3c_usb_phy_inited=1;
}

int s3c_usbctl_init(int ischeckstatus)/*diog, if not for check status, donot need wait cable insert*/
{
	u8 ucMode;
...
int s3c_udc_is_adc_cable()
{
	int count = 0;
	while(count<10)
		{
	if((readl(S5P_OTG_HPRT)&0xc00)==0xc00)
		count ++;
	else
		{
			count =0;
...
/*the following is for u-boot battery charge*/
int s3c_check_usb_connect_status()
{
	if(!s3c_usb_phy_inited)/*initalized at first*/
	{	
		s3c_usbctl_init(1);
	}
	/*then check current usb status*/
	unsigned long usec = (1000*1000*10);
	ulong tmo, tmp;
	...
	while (get_timer_masked()<tmo)	
	{
		...
		if(s3c_udc_is_adc_cable())
			s3c_usb_connected_status=2;
		if(s3c_usb_connected_status==2||s3c_usb_connected_status==1)
			break;
	}
	
	if(s3c_usb_phy_inited)
	{		
		s3c_usb_stop();
	}
	return s3c_usb_connected_status;
