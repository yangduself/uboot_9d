...
{
	struct block_allocation *alloc = ext4_malloc(sizeof(struct block_allocation));
...
{
	struct region *reg;
	reg = ext4_malloc(sizeof(struct region));
	reg->block = block;
	reg->len = len;
	reg->bg = bg_num;
	reg->next = NULL;
...
static void allocate_bg_inode_table(struct block_group_info *bg)
{
	...
	bg->inode_table = ext4_calloc(aux_info.inode_table_blocks, info.block_size);
	if (bg->inode_table == NULL)
...
static void init_bg(struct block_group_info *bg, unsigned int i)
{
	...
	bg->bitmaps = ext4_calloc(info.block_size, 2);
...
void block_allocator_init()
{
	...
	aux_info.bgs = ext4_calloc(sizeof(struct block_group_info), aux_info.groups);
...
void block_allocator_free()
{
	...
	for (i = 0; i < aux_info.groups; i++) {
		ext4_free(aux_info.bgs[i].bitmaps);
		ext4_free(aux_info.bgs[i].inode_table);
	}
	ext4_free(aux_info.bgs);
}

static u32 ext4_allocate_blocks_from_block_group(u32 len, int bg_num)
{
	if (get_free_blocks(bg_num) < len){
		error("failed to get_free_blocks %d %d",get_free_blocks(bg_num), len );
...
static struct region *do_split_allocation(struct block_allocation *alloc, u32 len)
{
	...
	if (len > 0) {
		new = ext4_malloc(sizeof(struct region));
...
struct ext4_inode *ext4_get_inode(u32 inode)
{
	inode -= 1;
