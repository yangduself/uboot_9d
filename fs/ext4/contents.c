	...
	u32 dirs)
{
	...
	blocks = EXT4_DIV_ROUND_UP(dentry_size(entries, dentries), info.block_size);
	...
	inode = ext4_get_inode(inode_num);
	if (inode == NULL) {
	...
	if (data == NULL) {
		error("failed to allocate %d extents", len);
		return EXT4_ALLOCATE_FAILED;
	}
...
{
	struct ext4_inode *inode = ext4_get_inode(inode_num);
