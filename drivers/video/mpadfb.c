...
#if(DISP_MODE==VGA_1280X1024)
#define S5PV210_CLK_SRC_MASK0 __REG(ELFIN_CLOCK_POWER_BASE + CLK_SRC_MASK0_OFFSET)
static void Disp_set_syscon_clk_fimd(void)
{
	...
	//关闭FIMD时钟输出
	cfg = S5PV210_CLK_SRC_MASK0 & ~(0x1<<5);
	S5PV210_CLK_SRC_MASK0 = cfg;
	
	//选择FIMD的时钟源为SCLKMPLL=667MHz
	cfg = CLK_SRC1_REG & ~(0xf<<20);
	CLK_SRC1_REG = cfg | (0x6<<20);
	
	//设置FIMD的分频系数，SCLK_FIMD = MOUTFIMD / (FIMD_RATIO + 1)，前面已经选择MOUTFIMD=SCLKMPLL，即SCLK_FIMD=SCLKMPLL/(FIMD_RATIO+1)
  	//SCLK_FIMD=667M/5=133M
	cfg = CLK_DIV1_REG & ~(0xf<<20);
	CLK_DIV1_REG = cfg | (0x4<<20);
	
	// Divider Status Check
	dwTimeOut = 10000;
	do
	{
	    dwReadTemp = CLK_DIV_STAT0_REG;
	    if(dwTimeOut-- == 0)
	    {
		 break;
	    }
	}while(dwReadTemp & (1<<13));
...
#if(DISP_MODE==VGA_1280X1024)
	// set clock source: sclk_mpll
	cfg = readl(S5PV210_VIDCON0);
	cfg &= ~(S5PV210_VIDCON0_CLKSEL_MASK | S5PV210_VIDCON0_CLKVALUP_MASK | S5PV210_VIDCON0_VCLKEN_MASK | S5PV210_VIDCON0_CLKDIR_MASK);
	cfg |= (S5PV210_VIDCON0_CLKSEL_SCLK | S5PV210_VIDCON0_CLKVALUP_ALWAYS | S5PV210_VIDCON0_VCLKEN_NORMAL | S5PV210_VIDCON0_CLKDIR_DIRECTED);
#else
	//get hclk for lcd
	hclk = get_HCLKD();
...
#if(DISP_MODE==VGA_1280X1024)
	Disp_set_syscon_clk_fimd();//lqm added.
#endif
